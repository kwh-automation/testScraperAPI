...
class EBRFRS < MCValidationTest
  ...
  def test_launching_a_form_from_the_phase
    ...
    @mc.batch_record_form_launch.select_form '1.1.1', form: @form
    @mc.batch_record_form_launch.launch phase_step: '1.1.1'
    assert @mc.phase.phase_steps[0].form_was_launched? '1.1.1'
    @launched_form = @mc.accountability.get_launched_form '1.1.1'
  end
  ...
  def test_verifying_form_launch_on_review_by_exception_page
    ...
    assert @mc.review_by_exception.form_status?('1.1.1')
  end

  def test_current_status_of_form_is_displayed
    assert @mc.review_by_exception.form_status('1.1.1') == @initial_status
    @mc.review_by_exception.toggle_gadget '2'
    assert @mc.review_by_exception.form_was_launched?('1.1.1')
    assert @mc.review_by_exception.launched_form('1.1.1') == @launched_form
    @mc.review_by_exception.open_launched_form '1.1.1'
    @mc.ztest_form01.page_one.form_number = '21'
    @mc.ztest_form01.sign_off 2
    @mc.use_last_window
    @mc.refresh
    @mc.review_by_exception.toggle_form
    @mc.review_by_exception.toggle_gadget '2'
    assert @mc.review_by_exception.form_status('1.1.1') == @second_step
  end
end
