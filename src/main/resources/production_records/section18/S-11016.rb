...
class EbrFRS < MCValidationTest
  ...
  def test_this
    ...
    pre_test
    test_irp_displays_pass_fail_data_type_severity
    test_irp_displays_accept_reject_configuration_with_running_total_defects
    test_irp_indicates_individual_pass_and_entire_production_record_pass
    test_irp_indicates_individual_fail_and_entire_production_record_fail
  end
  ...
  def pre_test
    ...
    @mc.manage_inspections.add_reject_count 1, row: 2
    @mc.go_to.ebr
  ...
  def test_irp_displays_pass_fail_data_type_severity
    assert @mc.inspection_review.severity_minor_element.attribute('innerText').include? "Minor"
    assert @mc.inspection_review.severity_major_element.attribute('innerText').include? "Major"
  end

  def test_irp_displays_accept_reject_configuration_with_running_total_defects
    assert @mc.inspection_review.accept_reject_threshold_element.exists?
    assert @mc.inspection_review.defect_count_element.exists?
    assert @mc.inspection_review.defect_total_element.exists?
  end

  def test_irp_indicates_individual_pass_and_entire_production_record_pass
    assert @mc.inspection_review.batch_overall_status == "Pass"
    individual =  @mc.inspection_review.get_individual_status "MAJOR"
    assert individual == "pass"
  end

  def test_irp_indicates_individual_fail_and_entire_production_record_fail
    ...
    @mc.inspection_review.open_severity_tab :minor
    @mc.inspection_review.open_severity_tab :major
    assert @mc.inspection_review.batch_overall_status == "Fail"
    new_individual = @mc.inspection_review.get_individual_status "MINOR"
    assert new_individual == "fail"
  end
